...
/**
 * 模拟小米加载进度 两个圆环 有一个跟随进度的圆在圆环上
 * https://github.com/baojie0327/ViewAndGroup
 */
public class CircleBarStepView extends View {
    //定义颜色
    ...
    public CircleBarStepView(Context context) {
        this(context, null);
    }

    public CircleBarStepView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CircleBarStepView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.XiaoMiStep_backGroundColor) { //背景颜色
                background_color = ta.getColor(attr, Color.WHITE); //默认为白色
            } else if (attr == R.styleable.XiaoMiStep_outerCircleColor) { //最外侧圆
                outer_circle_color = ta.getColor(attr, Color.WHITE);
            } else if (attr == R.styleable.XiaoMiStep_outerDotColor) { //最外侧圆上的小圆点
                outer_dot_color = ta.getColor(attr, Color.WHITE);
            } else if (attr == R.styleable.XiaoMiStep_lineColor) {  //最外侧线的颜色
                line_color = ta.getColor(attr, Color.WHITE);
            } else if (attr == R.styleable.XiaoMiStep_ringColor) { //圆环的颜色
                ring_color = ta.getColor(attr, Color.WHITE);
            } else if (attr == R.styleable.XiaoMiStep_stepNumColor) { //步数的颜色
                step_num_color = ta.getColor(attr, Color.WHITE);
            } else if (attr == R.styleable.XiaoMiStep_othetTextColor) { //其他文字颜色
