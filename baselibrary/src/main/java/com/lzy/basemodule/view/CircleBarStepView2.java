...
/**
 * 模拟小米加载进度 1个圆环 一个跟随进度的圆在圆环上
 *  * https://github.com/baojie0327/ViewAndGroup
 */
public class CircleBarStepView2 extends View {
    //定义颜色
    private int ring_color, dot_color,ring_progress_color,percent_color,
    ...
    public CircleBarStepView2(Context context) {
        this(context, null);
    }

    public CircleBarStepView2(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CircleBarStepView2(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.CircleProgress_ringColors) { //圆环颜色
                ring_color = ta.getColor(attr, Color.BLACK);
            } else if (attr == R.styleable.CircleProgress_ringSize) { //圆环宽度
                ringSize = ta.getDimension(R.styleable.CircleProgress_ringSize, 13);
            } else if (attr == R.styleable.CircleProgress_ringprogressColor) { //圆环进度颜色
                ring_progress_color = ta.getColor(attr, Color.WHITE);
            } else if (attr == R.styleable.CircleProgress_dotColor) {  //小圆点
                dot_color = ta.getColor(attr, Color.WHITE);
            } else if (attr == R.styleable.CircleProgress_dotSize) {  //小圆点大小
                dotSize = ta.getDimension(R.styleable.CircleProgress_dotSize, 32);
            } else if (attr == R.styleable.CircleProgress_textProgressColor) { //字体进度的颜色
                text_progress_color = ta.getColor(attr, Color.BLACK);
            } else if (attr == R.styleable.CircleProgress_textProgressSize) {  //字体进度大小
                textProgressSize = ta.getDimension(R.styleable.CircleProgress_textProgressSize, 32);
            } else if (attr == R.styleable.CircleProgress_textPercentColor) {  //百分号颜色
                percent_color = ta.getColor(attr, Color.BLACK);
            } else if (attr == R.styleable.CircleProgress_showProgressText) {  //固定字体显示
                showText = ta.getString(R.styleable.CircleProgress_showProgressText);
            } else if (attr == R.styleable.CircleProgress_texColor) { //字体的颜色
                text_color = ta.getColor(attr, Color.BLACK);
            } else if (attr == R.styleable.CircleProgress_texSize) {  //字体大小
                texSize = ta.getDimension(R.styleable.CircleProgress_texSize, 17);
            } else if (attr == R.styleable.CircleProgress_texMarginSize) {  //字之间的款阿杜
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //创建一个渲染器
        @SuppressLint({"DrawAllocation", "CanvasSize"}) SweepGradient mSweepGradient = new SweepGradient(canvas.getWidth() / 2
                , canvas.getHeight() / 2, new int[]{Color.rgb(140, 173, 248), Color.rgb(130, 120, 254), Color.rgb(129, 115, 251)}, null);
