 ...
 * 自定义控件
 */
public class MySpecButton extends View {
	 ...
	 * 控件文本
	 */
	private String _text = APPSingleton.getContext().getString(R.string.year_2013).toString();
	/**
	 * 字体颜色
	 */
	...
	public MySpecButton(Context context, String name) {
		super(context);
	...
	public MySpecButton(Context context, AttributeSet attrs) {
		super(context, attrs);
		...
		TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.MySpecButton);
		// String strText = array.getText(R.styleable.jwradiobtn_text, "");
		CharSequence text = array.getText(R.styleable.MySpecButton_android_text);
		if (text != null)
			_text = text.toString();

		textColor = array.getColor(R.styleable.MySpecButton_textColor, Color.BLACK); // 提供默认值，放置未指定
		textSize = array.getDimension(R.styleable.MySpecButton_textSize, 20);
		mPaint.setColor(textColor);
		mPaint.setTextSize(textSize);
	...
	public void onDraw(Canvas canvas) {
		...
		if (_text != null && !_text.equals("")) {
			...
			float baseY = this.getHeight() - (this.getHeight() - fontHeight) / 2 - fontMetrics.bottom;
		...
		if (_isPress) {
			...
			canvas.drawRect(1, 1, this.getWidth() - 1, this.getHeight() - 1, mPaint); // 绘制矩形
			return;
		}
	...
	private int measureWidth(int measureSpec) {
		...
		if (specMode == MeasureSpec.EXACTLY) {
			...
			// Measure the text
			result = (int) mPaint.measureText(_text) + getPaddingLeft() + getPaddingRight();
	...
	private int measureHeight(int measureSpec) {
		...
		if (specMode == MeasureSpec.EXACTLY) {
			...
			result = (int) (-_mAscent + mPaint.descent()) + getPaddingTop() + getPaddingBottom();
			if (specMode == MeasureSpec.AT_MOST) {
				// Respect AT_MOST value if that was what is called for by
				// measureSpec
				result = Math.min(result, specSize);
	...
	public int getRealWidth() {
		if (this.getWidth() == 0) {
			...
			return rect.width() + this.getPaddingLeft() + this.getPaddingRight();
		} else {
	...
	public int getRealHeight() {
		if (this.getHeight() == 0) {
			...
			return rect.height() + this.getPaddingTop() + this.getPaddingBottom();
		} else {
	...
	public interface ChangedCheckCallBackAttri {
		void ChangedCheck(MySpecButton attributeBtn);
	}
