...
public class GoodsList2Adapter extends BaseAdapter {
    ...
    public GoodsList2Adapter(Context mcontext, List<GoodsListModerInfo.datas.goods_list> goods_list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            convertView = View.inflate(mcontext, R.layout.item_goodslist2, null);
            new Goolist2Hodler(convertView);
        }
        Goolist2Hodler holder = (Goolist2Hodler) convertView.getTag();
        final GoodsListModerInfo.datas.goods_list goods = this.goods_list.get(position);

        RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams) holder.iv_goodsimg.getLayoutParams();
        layoutParams.height = (ClutterUtils.getScreenWidth(mcontext) - ClutterUtils.dp2px(mcontext, 20)) / 2;
        holder.iv_goodsimg.setLayoutParams(layoutParams);
...
//        holder.tv_name.setText(goods.goods_name);
    ...
    class Goolist2Hodler {
        ImageView iv_goodsimg;
        TextView tv_name, tv_price, tv_volume, tv_type;
        RelativeLayout rl_item;

        public Goolist2Hodler(View convertView) {
